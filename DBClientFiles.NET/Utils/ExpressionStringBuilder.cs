...
namespace DBClientFiles.NET.Utils
{
    internal sealed class ExpressionStringBuilder : ExpressionVisitor
    {
        ...
        private int GetLabelId(LabelTarget label)
        {
            ...
            if (_ids.TryGetValue(label, out var id)) return id;
        ...
        private static string FormatBinder(CallSiteBinder binder)
        {
            ...
            if (binder is ConvertBinder convert)
            {
                return "Convert " + convert.Type;
            }

            if (binder is GetMemberBinder getMember)
            {
                return "GetMember " + getMember.Name;
            }

            if (binder is SetMemberBinder setMember)
            {
                return "SetMember " + setMember.Name;
            }

            if (binder is DeleteMemberBinder deleteMember)
            {
            ...
            if (binder is InvokeMemberBinder call)
            {
            ...
            if (binder is UnaryOperationBinder unary)
            {
                return unary.Operation.ToString();
            }

            if (binder is BinaryOperationBinder binary)
            {
